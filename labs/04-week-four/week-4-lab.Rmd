...
1.  Use the function `summary()` after the regression to calculate the $R^2$ for both the total length and head length regressions. Recall that in the `possum` dataset, `total_l` is the total length of the possum, `head_l` is the head length, and `tail_l` is the tail length.
    ...
    R2_tot <- summary(lm(total_l ~ tail_l, data = possum))$r.squared

    R2_head <- summary(lm(head_l ~ tail_l, data = possum))$r.squared
...
The regression of the total length on tail length fits the data better and this is to be expected from the scatter plot because there is less variance? I am not sure.
    
$R^2$ is telling us how much of the variation of total length is dependent on tail length. 
$R^2$ is the percent of the variance in y that is explained by x. 
...
```{r}
summary(lm(hwy ~ year, data = mpg))

# to make this into a table 
...
Because in the formula $hwy_i=\beta_{0}+\beta_{1} \cdot vintage_i +\varepsilon_i$, $\beta_0$ is the intercept, the predicted highway miles per gallon for a car in 1999 is 23.4mpg. 

3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words?

Fuel efficiency is an average of 0.02 mpg higher for a car in 2008 than a car in 1999.  

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

Fuel efficiency has likely increased over time because there is a slight increase in the average miles per gallon on a car from 1999 to 2008, however this is tiny and unreliable. 
...
It appears that as the engine size increases, the highway fuel efficiency decreases. 
...
```{r}
summary(lm(hwy ~ displ + year, data = mpg))
```

2.  Interpret your three coefficients, paying careful attention to units.

The estimated highway fuel efficiency for a car in 1999 without engine size as a factor (so displacement size in liters is 0) is 35.3. You should be cautious with this because this is implying that there is basically no engine. 

The engine displacement coefficient tells us that as the highway fuel efficiency increases, the engine displacement in litres decreases by 3.6. 

The 2008 coefficient tells us that as highway fuel efficiency increases, without factoring in engine size here, there is an increase of 1.4 in fuel efficiency in the year 2008. 
...
This coefficient for the year 2008 is larger here than the first regression, so this suggests that engine displacement in litres has an impact on the fuel efficiency from 1999 to 2008. 
