...
loans_clean <- loans_full_schema %>% 
  mutate(
    ...
    homeownership = fct_relevel(homeownership, "rent"), #reorder with rent as the highest factor
...
p_count + p_proportion # this is a quick way to see the graphs side by side but if you want to customize it or share a legend or anything, you should use patchwork. 
...
```{r}
p_fill <- ggplot(loans_clean, aes(x = homeownership, fill = application_type)) +
...
p_standardized <- ggplot(loans_clean, aes(x = homeownership, fill = application_type)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = c(IMSCOL["blue", "full"], IMSCOL["yellow", "full"])) +
